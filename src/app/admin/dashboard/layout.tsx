...
import { AdminGuard } from "@/components/auth/AdminGuard"; // IMPORTANTE
import {
  Bus,
  LayoutDashboard,
  MapPin,
...
export default function AdminLayout({
  ...
  router.refresh(); 
  router.push("/admin/login"); // <-- Antes decía /login a secas
};

  // ACTUALIZACIÓN DE MENÚ: Coherente con gestión privada
  const menuItems = [
  { name: "Resumen", icon: LayoutDashboard, path: "/admin/dashboard" },
  { name: "Usuarios", icon: Users, path: "/admin/dashboard/usuarios" },
  { name: "Cooperativas", icon: Building2, path: "/admin/dashboard/cooperativas" },
  ...
  return (
    <AdminGuard> {/* PROTECCIÓN DE NIVEL LAYOUT */}
      <div className="min-h-screen bg-[#F8FAFC] flex h-screen overflow-hidden font-sans">
        
        <Toaster position="bottom-right" richColors theme="light" 
          toastOptions={{
            style: { borderRadius: '1.5rem', border: '1px solid #f1f5f9', fontFamily: 'inherit' },
        ...
        <aside className={`fixed inset-y-0 left-0 z-[60] w-72 bg-[#09184D] text-white transition-all duration-500 ease-in-out lg:static lg:translate-x-0 ${isSidebarOpen ? "translate-x-0 shadow-2xl" : "-translate-x-full"}`}>
          <div className="h-full flex flex-col p-6">
            ...
            <nav className="flex-1 space-y-1.5 overflow-y-auto pr-2 custom-scrollbar">
              {menuItems.map((item) => {
  // LÓGICA DE ACTIVACIÓN DEPURADA
  const isActive = 
    item.path === "/admin/dashboard" 
      ? pathname === "/admin/dashboard" // Para el Resumen, solo si es la ruta exacta
      : pathname.startsWith(item.path); // Para los demás, si la ruta comienza con el path
  ...
  return (
    <Link 
      ...
      className={`flex items-center justify-between px-5 py-4 rounded-2xl transition-all duration-300 group 
        ${isActive 
          ? "bg-gradient-to-r from-[#EA2264] to-[#ff4d88] text-white shadow-lg shadow-[#EA2264]/20 scale-[1.02]" 
          ...
          <header className="bg-white/70 backdrop-blur-xl border-b border-gray-100 h-20 flex items-center justify-between px-6 md:px-10 shrink-0 z-40">
            ...
            <div className="flex items-center gap-6">
              <div className="text-right hidden sm:block">
                <p className="text-[11px] font-black text-[#09184D] uppercase italic leading-none">{user?.email?.split("@")[0] || "Admin"}</p>
              ...
              <div className="relative" ref={settingsRef}>
                <button onClick={() => setShowSettings(!showSettings)} className={`w-12 h-12 rounded-2xl flex items-center justify-center border transition-all duration-500 cursor-pointer ${showSettings ? "bg-[#09184D] border-[#09184D] text-white shadow-lg" : "bg-gray-50 border-gray-100 text-slate-400 hover:border-[#EA2264]/30"}`}>
