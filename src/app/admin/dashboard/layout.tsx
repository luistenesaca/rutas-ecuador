...
import { 
  Bus, LayoutDashboard, MapPin, Users, LogOut, 
  Menu, ChevronRight, Settings, Route, Loader2, X, User, ShieldCheck
} from "lucide-react";
...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] flex h-screen overflow-hidden font-sans">
      ...
      {/* SIDEBAR */}
      <aside className={`
        fixed inset-y-0 left-0 z-[60] w-72 bg-[#09184D] text-white transition-all duration-500 ease-in-out lg:static lg:translate-x-0
      ...
      `}>
        <div className="h-full flex flex-col p-6">
          ...
          <nav className="flex-1 space-y-1.5 overflow-y-auto pr-2 custom-scrollbar">
            {menuItems.map((item) => {
              // CORRECCIÓN: Resumen requiere ruta exacta, los demás aceptan sub-rutas
        ...
        <header className="bg-white/70 backdrop-blur-xl border-b border-gray-100 h-20 flex items-center justify-between px-6 md:px-10 shrink-0 z-40">
          ...
          <div className="flex items-center gap-6">
             <div className="text-right hidden sm:block">
                ...
                <p className="text-[8px] text-emerald-500 font-black uppercase tracking-widest mt-1 flex items-center justify-end gap-1">
                  <span className="w-1 h-1 rounded-full bg-emerald-500 animate-pulse"></span> Sesión Activa
                </p>
             </div>
             ...
             <div className="relative" ref={settingsRef}>
                <button 
                  ...
                  className={`w-12 h-12 rounded-2xl flex items-center justify-center border transition-all duration-500 cursor-pointer 
                  ...
                  <div className="absolute right-0 mt-3 w-64 bg-white border border-gray-100 shadow-2xl rounded-[2rem] p-4 animate-in fade-in zoom-in-95 duration-200">
                    ...
                    <div className="space-y-1">
                      <button 
                        ...
                        className="w-full flex items-center gap-3 px-4 py-3 text-[10px] font-black text-[#09184D] uppercase tracking-widest hover:bg-gray-50 rounded-xl transition-all"
                      >
      ...
      {isSidebarOpen && (
        <div 
          className="fixed inset-0 bg-[#09184D]/60 z-[55] lg:hidden backdrop-blur-sm transition-opacity" 
          onClick={() => setIsSidebarOpen(false)} 
        />
      )}
    </div>
